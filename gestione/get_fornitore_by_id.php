...
$arr_cliente = $manager_sql->get_fornitore_by_id($id_cliente);
...
if(!empty($arr_cliente)){
	...
	for($i=0; $i<$cnt; $i++){
		...
		$sito = $arr_cliente[$i][12];
                $tipo = $arr_cliente[$i][13];
		
$toReturn = $rag_soc."(:)".$indirizzo."(:)".$citta."(:)".$cap."(:)".$piva."(:)".$cf."(:)".$referente."(:)".$mail."(:)".$tel."(:)".$cell1."(:)".$cell2."(:)".$sito."(:)".$tipo;
