...
$arr_cliente = $manager_sql->get_newsletter_by_id($id_cliente);
...
if(!empty($arr_cliente)){
	...
	for($i=0; $i<$cnt; $i++){
		...
		$sito = $arr_cliente[$i][12];
                $provincia = $arr_cliente[$i]['provincia'];
		
$toReturn = $rag_soc."(:)".$indirizzo."(:)".$citta."(:)".$cap."(:)".$piva."(:)".$cf."(:)".$referente."(:)".$mail."(:)".$tel."(:)".$cell1."(:)".$cell2."(:)".$sito."(:)".$provincia;
