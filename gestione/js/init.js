...
$(function(){
	...
	$('#btn_nuovo_operatore').click(
		function(){
			var dati = $('#form_nuovo_operatore').formSerialize();
	...
	$( "#dialog-form" ).dialog({
			...
			open: function() {
				...
				$.post('get_operatore_by_id.php', { id_operatore : id_op }, function(data){
					...
					$('#nome_operatore_dg').val(decodeURIComponent(arr_operatore[0])); //<-val(decodeURIComponent(arr_operatore[0]))
					$('#cognome_operatore_dg').val(decodeURIComponent(arr_operatore[1]));
					//$('#interno_dg').val(arr_operatore[2]);
					$('#interno_dg').val(decodeURIComponent(arr_operatore[2]));
					//$('#email_operatore_dg').val(arr_operatore[7]);
					$('#email_operatore_dg').val(decodeURIComponent(arr_operatore[7]));
					//$('#cell1_dg').val(arr_operatore[4]);
					//$('#cell2_dg').val(arr_operatore[5]);
					//$('#id_op').val(id_op);
					$('#cell1_dg').val(decodeURIComponent(arr_operatore[4]));
					$('#cell2_dg').val(decodeURIComponent(arr_operatore[5]));
					$('#id_op').val(decodeURIComponent(id_op));
		...
		$('#dialog-form-modifica-schedalavoro').dialog({
			...
			open: function() {
				id_scheda = $(this).data('id_scheda'); //<------------------- funzione che prende l'id della scheda
		...
		/** MODIFICA BY ADMIN */ /** MODIFICA DELLA DIMENSIONE 680*500px SCHEDA A COMPARSA NELLA TAB HOME */
