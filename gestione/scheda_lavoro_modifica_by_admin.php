...
$settore = $_POST['settore_to_scheda_lavoro'];

$data_consegna_arr = explode('/',$data_consegna);
$data_consegna = $data_consegna_arr[2].'/'.$data_consegna_arr[1].'/'.$data_consegna_arr[0];

/*$debug = $manager_sql->update_schedaLavoro_by_admin($id_scheda,$stato, $idoperatore, $titolo, $note, $data_consegna, $priorita);
echo $debug;
*/

if($manager_sql->update_schedaLavoro_by_admin($id_scheda,$stato, $idoperatore, $titolo, $note, $data_consegna, $priorita, $settore,$costo))
	echo '1';
else
	echo '0';
